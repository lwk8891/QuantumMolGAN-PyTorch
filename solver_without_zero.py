...
from utils.dfs import is_connected_without_zero
...
class Solver(object):
    ...
    def get_reward(self, n_hat, e_hat, method):
        ...
        edges_hard, nodes_hard = torch.max(edges_hard, -1)[1], torch.max(nodes_hard, -1)[1]
        mols = [self.data.matrices2mol(n_.data.cpu().numpy(), e_.data.cpu().numpy(), strict=True) for e_, n_ in zip(edges_hard, nodes_hard)]
        connected = np.array([is_connected_without_zero(n_.data.cpu().numpy(), e_.data.cpu().numpy(), 0) for e_, n_ in zip(edges_hard, nodes_hard)])
    ...
    def train_or_valid(self, epoch_i, train_val_test='val'):
        ...
        for a_step in range(the_step):
            ...
            assert not (np.isnan(loss_D.item()) or np.isnan(d_loss_fake.item()) or np.isnan(d_loss_real.item()) or np.isnan(loss_G.item()))
